...
class RepeatIteratorTest extends IteratorsTestBase
{
    ...
    public function testRepeatIteratorCountable()
    {
        $input = ['test1', 'test4', 'test2', 'test3'];

        $iterator = new RepeatIterator(new \ArrayIterator($input), 3);
        $this->assertCount(12, $iterator, 'Iterator was not counted correctly.');

        $iterator = new RepeatIterator(new \IteratorIterator(new \ArrayIterator($input)), 3);
        $iterator->rewind();
        $iterator->next();
        $this->assertCount(12, $iterator, 'Iterator was not counted correctly.');
    }

    public function testRepeatIteratorNonCountable()
    {
        $input = ['test1', 'test4', 'test2', 'test3'];
